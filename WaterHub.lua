...
if Humanoid.RigType == Enum.HumanoidRigType.R15 then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[WaterHubV1] You are on R15! Please use the R15 Reanimation instead.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
return end
if game.Players.LocalPlayer.Character:FindFirstChild("WaterHubV1") or workspace:FindFirstChild("WaterHub V1") then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[WaterHub V1] You are already reanimated! Please reset to unreanimate.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
...
game:WaitForChild("Run Service").Heartbeat:Connect(function()
    if Reanimated == true then
        if Character.WaterHubV1 then
            Character.Torso.CFrame=Character.WaterHubV1.Torso.CFrame
        end
...
CopyCharacter.Name = "WaterHubV1" -- you can always change name.

for k,l in pairs(Character.WaterHubV1:GetDescendants()) do 
...
game:GetService("UserInputService").JumpRequest:connect(function(t)
    if CopyCharacter.Humanoid.FloorMaterial~=Enum.Material.Air then 
        ...
        game.Players.LocalPlayer.Character.WaterHubV1:FindFirstChildOfClass('Humanoid').Sit=false 
    end 
end)
